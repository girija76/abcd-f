...
(function() {
	...
	Na.Chart = (function() {
		...
		function v(a, d) {
			for (var b = [], c, e = 0; e < a.length; e++)
				if (0 == e) b.push(a[0]);
					...
					c =
						(Math.abs(
							(a[m].x - a[g].x) / (0 === a[m].x - a[l].x ? 0.01 : a[m].x - a[l].x)
						) *
							(d - 1)) /
							2 +
						1;
					...
					c =
						(Math.abs(
							(a[m].x - a[g].x) / (0 === a[l].x - a[g].x ? 0.01 : a[l].x - a[g].x)
						) *
							(d - 1)) /
							2 +
						1;
		...
		function I(a, d, b, c, e) {
			...
			a =
				parseFloat(a.toString()) * (0 <= a.toString().indexOf('%') ? d / 100 : 1);
			'undefined' !== typeof c &&
								...
								for (w = 0; w < k.dataPoints.length; w++)
									if (
										...
										var r =
											((n + 1 - m) *
												Math.max(
													parseFloat(a.axisY.scaleBreaks.collapsibleThreshold) || 10,
													10
												)) /
											100;
										if (
			...
			for (var c, e = this, g = !1, m = 0; m < this._axes.length; m++)
				if (
					...
					for (
						...
						100 /
							Math.max(
								parseFloat(this._axes[m].scaleBreaks.collapsibleThreshold) || 10,
								10
							);
						l++
					)
							...
							? ((e = b.y1),
							  ...
							  0 < ma
									...
									: ((n = h.point.y + m + c),
									  n > g - w - m - c &&
											((n =
												'auto' === t ? Math.min(h.point.y, g) - w - m - c : g - w - m - c),
											(v = n < h.point.y))))
								...
								? ((m = b.x1),
								  ...
								  0 > ma
										...
										: ((q = h.point.x + e + c),
										  q > g - l - e - c &&
												((q =
													'auto' === t ? Math.min(h.point.x, g) - l - e - c : g - l - e - c),
												(v = q < h.point.x))))
					...
					if (0 < t.length)
						for (var u = 5 < f && k.bevelEnabled ? !0 : !1, h = 0; h < t.length; h++)
							if (
								...
								s = a.axisX.convertValueToPixel(c);
								q =
									0 !== a.dataPointYSums[c] ? 100 * (t[h].y / a.dataPointYSums[c]) : 0;
								var x = (s - (a.plotType.plotUnits.length * f) / 2 + a.index * f) << 0,
									v = (x + f) << 0,
									y;
								if (
		...
		p.prototype.renderPie = function(a) {
			...
			function e(a) {
				for (var b = null, e = 1; e < s.length; e++)
					if (
						((b = (a + e + f.length) % f.length), f[b].hemisphere !== f[a].hemisphere)
					) {
						b = null;
						break;
					} else if (
					...
					for (a = 0; a < s.length; a++)
						if (
							...
							(y = y = 0),
								'right' === l.hemisphere
									? ((y =
											n.x2 - (l.indexLabelTextBlock.x + l.indexLabelTextBlock.width + q)),
									  (y *= -1))
									: (y =
											n.x1 - (l.indexLabelTextBlock.x - l.indexLabelTextBlock.width - q)),
								0 < y &&
									(!m &&
										l.indexLabelText &&
										((u =
									...
									Math.abs(
										l.indexLabelTextBlock.y - l.indexLabelTextBlock.height / 2 - x.y
									) < A ||
										Math.abs(
											l.indexLabelTextBlock.y + l.indexLabelTextBlock.height / 2 - x.y
										) < A) &&
								...
								0 < l.indexLabelAngle && l.indexLabelAngle < Math.PI
									? ((y =
											n.y2 -
											(l.indexLabelTextBlock.y + l.indexLabelTextBlock.height / 2 + 5)),
									  (y *= -1))
									: (y =
											n.y1 -
											(l.indexLabelTextBlock.y - l.indexLabelTextBlock.height / 2 - 5)),
		...
		p.prototype.renderFunnel = function(a) {
			...
			function c() {
				if (t && C) {
					for (
						...
						(w = P[y]),
							(w =
								'undefined' !== typeof w.x5 ? (w.y2 + w.y4) / 2 : (w.y2 + w.y3) / 2),
							(w = f(w).x2 + 1),
							(p[y] = L - w - S);
					w = 0.5 * S;
				...
				if (d)
					for (h = J.length - 1; 0 <= h; h--)
						(a = P[h]),
							...
							(J[h].width = d),
							(c =
								'undefined' !== typeof a.x5 ? (a.y2 + a.y4) / 2 : (a.y2 + a.y3) / 2),
					...
					for (d = 0; d < P.length; d++)
						0 == J[d].textBlock.text.length
							...
							: ((a = P[d]),
							  ...
							  b >= c
									? ((b =
											d != fa ? (a.x4 + a.x3) / 2 - e / 2 : (a.x5 + a.x4) / 2 - e / 2),
									  (c =
											d != fa ? (a.y1 + a.y3) / 2 - c / 2 : (a.y1 + a.y4) / 2 - c / 2),
									  (J[d].textBlock.x = b),
									  (J[d].textBlock.y = c))
									: (J[d].isDirty = !0));
			}
			...
			function k(a, b, c) {
				...
				if (0 === W.length) l = ia;
					...
					e =
						(D * (1 != W.length || (0 != W[0] && W[0] != P.length - 1) ? 2 : 1)) /
						h();
					...
					if (-1 !== I) {
						...
						this.funnelPyramidClickHandler = function(a) {
							...
							if (
								...
								!z.isAnimating &&
								(u(a.dataSeries.explodeOnClick) || a.dataSeries.explodeOnClick) &&
								((b = t.reversed ? C.length - 1 - a.dataPointIndex : a.dataPointIndex),
								0 <= b)
							) {
								a = b;
								if ('funnel' === t.type || 'pyramid' === t.type)
									t.reversed
										? (C[C.length - 1 - a].exploded = C[C.length - 1 - a].exploded
												? !1
												: !0)
				...
				if (q.dataPoints && q.dataPoints.length)
					if (
						...
						var n = (q.legendMarkerType = q.legendMarkerType
							...
							r =
								q.markerSize ||
								('line' !== q.type && 'stepLine' !== q.type && 'spline' !== q.type)
					...
					: ((s =
					  ...
					  (s =
							0 < s
								? Math.min(
										Math.max(
											(((this.dataPoints.length - 1) / s) * (a - this.dataPoints[0].x)) >>
												0,
											0
										),
										this.dataPoints.length
					...
					(n = this.dataPoints[this.dataPoints.length - 1].x - this.dataPoints[0].x),
						(q =
							0 < n
								? Math.min(
										Math.max(
											(((this.dataPoints.length - 1) / n) * (q - this.dataPoints[0].x)) >>
												0,
											0
										),
										this.dataPoints.length
						...
						switch (this.type) {
							...
							case 'doughnut':
								...
								p < n.radius &&
									...
									(r = Number(
										((((180 * (n.endAngle / Math.PI)) % 360) + 360) % 360).toFixed(12)
									)),
							...
							if (!this._labels[b].breaksLabelType) {
								...
								if (
									...
									if (
										...
										(p =
											(h -
												(k + a.fontSize / 2) * Math.cos((Math.PI / 180) * Math.abs(-25))) /
										...
										if (u(this.options.labelWrap))
											if (!u(this.options.labelMaxWidth))
												...
												if (
													((c = (B.width + d.width) >> 0), (f = this.labelFontSize), n < g)
												)
													c - 2 * g > q &&
														((q = c - 2 * g),
														c >= 2 * g && c < 2.2 * g
						...
						for (f = 0; f < a.length; f++)
							a[f] &&
								...
								a[f]._labels &&
									...
									(m =
										a[f]._labels[
											a[f]._labels.length - ('dateTime' === a[f].valueType ? 2 : 1)
										].position),
						...
						for (f = 0; f < d.length; f++)
							d[f].calculateValueToPixelConversionParameters(),
								...
								d[f]._labels &&
									...
									(m =
										d[f]._labels[
											d[f]._labels.length - ('dateTime' === d[f].valueType ? 2 : 1)
										].position),
				...
				if (
					...
					this.stripLines[a].value = this.logarithmic
						? this.stripLines[a].value *
						  Math.sqrt(
								Math.log(this.stripLines[a].endValue / this.stripLines[a].startValue) /
									Math.log(d)
						  )
						: this.stripLines[a].value +
		...
		z.prototype.calculateBreaksInPixels = function() {
			if (!(this.scaleBreaks && 0 >= this.scaleBreaks._appliedBreaks.length)) {
				...
				for (
					var d = 0;
					d < a.length && !(a[d].startValue > this.conversionParameters.maximum);
					d++
				)
								...
								? ((n.x -= q.textBlock.width / 2),
								  (n.y =
										'inside' === this.labelPlacement
											...
											: n.y -
											  (this.tickLength + q.textBlock.height - q.textBlock.fontSize / 2)))
								: ((n.x =
										'inside' === this.labelPlacement
											? 0 < this.labelAngle
												? n.x
		...
		z.prototype.calculateLogarithmicAxisParameters = function() {
			...
			if (
				!this.options.interval &&
				this.range <
					Math.pow(this.logarithmBase, 8 > this.viewportMaximum || 3 > a ? 2 : 3)
			) {
				for (
			...
			if (this.snapToDataPoint) {
				...
				if ('xySwapped' === this.chart.plotInfo.axisPlacement) {
					...
					if (
						...
						for (
							f = Math.abs(d - this.parent.convertValueToPixel(m[0].dataPoint.y[0])),
								n = v = 0;
							n < m.length;
							n++
						)
							...
							if (h.dataSeries.axisY)
								if (0 < h.dataPoint.y.length) {
									...
									'column' === e.type || 'bar' === e.type
										...
										: 'bubble' === e.type
										? ((b =
												this.chart._eventManager.objectMap[e.dataPointIds[h.index]].size /
												2),
										...
										? ((b = 0),
										  h.cumulativeSumYStartValue < h.dataSeries.axisY.viewportMinimum
												? b--
												: h.cumulativeSumYStartValue > h.dataSeries.axisY.viewportMaximum &&
												  b++,
