...
function getAssessmentStats(assessment, aNames, accuracy) {
	console.log('check assessment stats', assessment);
...
const SessionDetailDesktop = ({
	...
	console.log('check activePhase old..', activePhase);
	...
	console.log('check additional info', sessionTypes, sessionType, session);
	...
	return (
		<Card
			title={
				<div style={{ display: 'flex', alignItems: 'center' }}>
					<Link
						to={URLS.activitySession}
						style={{ marginRight: 16, padding: '1px 8px' }}
					>
						<ArrowLeftOutlined />
					</Link>
			...
			style={{ border: 0 }}
			headStyle={{ border: 0, fontSize: 20, padding: 0 }}
			bodyStyle={{
				padding: 0,
				display: 'flex',
		...
		>
			...
			<div className="note-wrapper-desktop" style={{ marginTop: 18 }}>
...
const mapStateToProps = (state, ownProps) => {
	...
	return {
		sessionId,
		session,
		activePhase: ownProps.activePhase,
